...
RegisterNetEvent('esx:copyCoords', function(type)
	local coords = GetEntityCoords(ESX.PlayerData.ped)
	local heading = GetEntityHeading(ESX.PlayerData.ped)
	local stringCoords
	if type == '3' then
		stringCoords = ('vec3(%.4f, %.4f, %.4f)'):format(coords.x, coords.y, coords.z)
	elseif type == '4' then
		stringCoords = ('vec4(%.4f, %.4f, %.4f, %.4f)'):format(coords.x, coords.y, coords.z, heading)
	end
	SendNUIMessage({
		coords = stringCoords
	})
